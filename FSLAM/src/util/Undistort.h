...
public:
	...
	PhotometricUndistorter(std::string file, std::string noiseImage, std::string vignetteImage, Undistort* geomUndist ,int w_, int h_);
...
public:
	...
	template<typename T>
	float* geometricallyUnDistortVignette(const MinimalImage<T>* image_raw);
	float* geometricallyUnDistortVignette(const MinimalImageB *image_raw);
